    ...
    if params[:filter]
      case params[:filter]
        when 'speakers'
          ...
          @companies = User.find_by_sql("SELECT company, count(*) AS num_of_participants FROM (SELECT DISTINCT u.name, u.company FROM users u INNER JOIN speakers s ON u.id = s.user_id INNER JOIN talks t ON t.id = s.talk_id ORDER BY u.company, u.name) GROUP BY company ORDER BY num_of_participants DESC, company")
        when 'accepted'
