...
class RegistrationsControllerTest < ActionController::TestCase
  ...
  def test_receipt
    user = login_as(:paid_participant)
    get :receipt, :id => user.registration.id
    assert_response 200
  end


  def test_receipt_access_denied
    user = login_as(:paid_participant)
    get :receipt, :id => users(:quentin).registration.id
    assert_response 302
  end

  def test_receipt_unpaid
